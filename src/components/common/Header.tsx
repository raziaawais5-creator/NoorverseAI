...
import { Sun, Moon, Search, BookOpen, Compass, Sparkles, User as UserIcon, LogIn, Smartphone } from 'lucide-react';
...
export const Header: React.FC<{
  ...
  isFramed?: boolean;
  toggleFrame?: () => void;
}> = ({ onOpenSearch, isFramed, toggleFrame }) => {
  const { isDarkMode, toggleDarkMode } = useTheme();
  const { activeTab, setActiveTab, isPremium, openUpgradeModal, userCurrency, user, openAuthModal } = useApp();
    ...
    <header className="sticky top-0 z-30 bg-[#F7F9FC]/90 dark:bg-[#0B1320]/90 backdrop-blur-md border-b border-[#C7CEDB]/40 dark:border-[#1F3A5F]/60 px-4 py-3 transition-colors duration-200">
      <div className="max-w-5xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveTab('home')}>
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-[#1F3A5F] via-[#2C5282] to-[#6E8FB5] p-0.5 shadow-md flex items-center justify-center relative group">
            <div className="w-full h-full bg-[#1F3A5F]/20 rounded-2xl flex items-center justify-center">
              <span className="text-[#F7F9FC] font-bold text-xl tracking-wider select-none">ن</span>
            </div>
          ...
          <div>
            <div className="flex items-center space-x-1.5">
              <h1 className="text-lg font-bold tracking-tight text-[#1F3A5F] dark:text-[#F7F9FC]">
                NoorVerse
              </h1>
              {isPremium ? (
            ...
            <p className="text-xs text-[#6E8FB5] dark:text-[#C7CEDB]/70 font-medium">
              14 Safar 1448 AH • Peaceful Companion
            </p>
          </div>
        </div>
